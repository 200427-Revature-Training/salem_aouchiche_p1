...
export async function getAllReimbursements():Promise<Reimbursement[]> {
    // console.log('getAllReimbursements');
     ...
     LEFT JOIN ERS_REIMBURSEMENT_TYPE ERT ON ER.REIMBURSEMENT_TYPE_ID = ERT.ERS_REIMBURSEMENT_TYPE_ID `; 
      ...
      const sql= 
    ` SELECT ER.REIMBURSEMENT_AMOUNT, EU.USER_FIRST_NAME AS Author_FirstName, EU.USER_LAST_NAME AS Author_LastName,
      ER.REIMBURSEMENT_DESCRIPTION, ER.REIMBURSEMENT_RECEIPT, EU2.USER_FIRST_NAME AS resolver_FirstName, EU2.USER_LAST_NAME AS resolver_LastName,
